...
def log(value, status='Jaka', color='[P1]'):
...
def log_replace(value, status='Jaka', color='[G1]'):
...
class domain_fronting(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.log('TERHUBUNG BOI :) !!!'.format(self.proxy_host, self.proxy_port), color='[G1]')
        except OSError:
...
def main():
    ...
    if user_input != ip:
        sys.exit(' PASSWORD SALAH SILAHKAN ULANG\n')
    print ' BERHASIL!!!\n'
